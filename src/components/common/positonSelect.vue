	...
	<span class="positonSelect">
		...
		<div class="selectPanel" v-show="message">
			<div class="filter">
				...
				<input @focus="showPositionSelect" v-model="keyword" id="tipinput" class="mgl10" placeholder="您要去哪儿" type="" name="">
			...
			<div class="body">
				...
				<div class="placeSelect" v-if="isPositionSelectShow">
					<div v-for="item in tips" class="placeItem clearfix" @click.stop='placeSelected(item)'>
						<div class="fl mgr10">
							<img :src="imgPath+'deleteLater.png'" class="icon mgt5">
						</div>
						<div class="fl">
							<div>{{item.name}}</div>
							<div class="fontGrey">{{item.address}}</div>
						</div>
...
export default {
	...
	methods:{
	    ...
	    searchByKeyword(){
	    	var _this=this;
			this.auto.setCity(this.selectedCityName);
			this.auto.search(this.keyword,function(status,info){
				if (status=="complete") {
